...
2、梯度最大，学习最快
第四步：对注意力分数矩阵进行 softmax 操作，得到注意力权重矩阵。
...
1、如果句子长度不同，需要对句子进行填充，使句子句子长度相同。（为了在训练时，每个句子的长度都相同，方便计算）
例如：
...
###三、前馈网络
最简单的人工神经网络，信息单向向前传递，无循环、无反馈、无跨层反向连接；数据从输入层 → 隐藏层 → 输出层，一层计算完输出传给下一层，不会回传。
也叫多层感知机 MLP（多层前馈网络）。
###四、交叉注意力机制
有两部分输入，一部分是来自编码器的K和V，一部分是解码器的Q。
作用：解码器每生成一个 token，都去编码器里找最相关的源信息，实现序列对齐。
